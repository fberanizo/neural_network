...
class GlobalInternalRNN(unittest.TestCase):
    ...
    def test_1(self):
        """Tests the accuracy of a Global Internal RNN using k-folds validation method."""
        ...
        for fold, (train_index, test_index) in enumerate(skf):
            ...
            for idx, hiperparams in enumerate(self.hipergrid()):
                ...
                for train_index2, test_index2 in skf2:
                    ...
                    classifier2 = rnn.GlobalInternalRNN(**hiperparams).fit(X_train2, y_train2)
            ...
            classifier = rnn.GlobalInternalRNN(**best_hiperparams).fit(X_train, y_train)
            accuracies_test[fold] = classifier.score(X_test, y_test)

        print 'Acurácia média:' + numpy.mean(accuracies_test)

    def hipergrid(self):
        """Hiperparameters for Global Internal RNN"""
